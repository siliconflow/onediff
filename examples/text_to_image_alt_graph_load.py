...
from onediff import OneFlowAltDiffusionPipeline as AltDiffusionPipeline
from diffusers import utils

_model_id = "BAAI/AltDiffusion-m9"
...
def _test_sd_graph_save_and_load(is_save, graph_save_path, sch_file_path, pipe_file_path):
    ...
    def get_pipe():
        if _pipe_from_file:
            scheduler = DPMSolverMultistepScheduler.from_pretrained(sch_file_path, subfolder="scheduler")
            sd_pipe = AltDiffusionPipeline.from_pretrained(
                pipe_file_path, scheduler=scheduler, torch_dtype=torch.float16
                )
        else:
            scheduler = DPMSolverMultistepScheduler.from_pretrained(_model_id, subfolder="scheduler")
            sd_pipe = AltDiffusionPipeline.from_pretrained(
                _model_id, scheduler=scheduler, torch_dtype=torch.float16
                )
    ...
    def text_to_image(prompt, image_size, num_images_per_prompt=1, prefix="", with_graph=False):
        ...
        cur_generator = torch.Generator("cuda").manual_seed(0)
        images = pipe(
            prompt,
            height=image_height,
            width=image_weight,
