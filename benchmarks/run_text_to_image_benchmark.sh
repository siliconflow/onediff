...
while getopts 'mwco:h' opt; do
  case "$opt" in
    ...
    ?|h)
      ...
      echo "  -w warmups: the number of warmups, default is 3"
      echo "  -c compiler: the compiler, default is oneflow"
      echo "  -o output_file: the output file, default is stdout"
      exit 1
      ;;
  esac
done
...
BENCHMARK_RESULT_TEXT="| Model | Inference Time (s) | Iterations per second | CUDA Mem after (GiB) | Host Mem after (GiB) |\n| --- | --- | --- | --- | --- |\n"
