...
from .utils.optimization_tools import (get_model_type, is_fp16_model,
...
class BasicOneFlowOptimizerExecutor(OptimizerExecutor):
    ...
    def _(self, model: ModelPatcher, ckpt_name: Optional[str] = None, **kwargs):      
        ...
        if not is_fp16_model(torch_model):
            print(f'Warning: Model {type(torch_model)} is not an FP16 model. Compilation will be skipped!')
            return model
    
        compiled_model = oneflow_compile(torch_model)
        model.model.diffusion_model = compiled_model
        if ckpt_name:
            graph_file = generate_graph_path(f"{ckpt_name}_{type(model.model).__name__}", torch_model)
    ...
    def _(self, model, ckpt_name: Optional[str] = None, **kwargs) -> ControlNet:
        ...
        if not is_fp16_model(torch_model):
            ...
            print(f'Warning: Model {type(torch_model)} with parameter types {type_set} is not an FP16 model. Compilation will be skipped!')
            return model
    ...
    def _(self, model, ckpt_name: Optional[str] = None, **kwargs)->VAE:
        ...
        if gpu_name == 'NVIDIA A800-SXM4-80GB':
            ...
            print(f'Warning: Model {model_type} not supported on {gpu_name}. Compilation skipped!')
            return model
