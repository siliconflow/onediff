...
def unpatch_model_oneflow(org_fn, self, device_to=None, unpatch_weights=True):
    if unpatch_weights:
        logger.warning(f"{type(self.model.diffusion_model)} is quantized by onediff, so unpatching is skipped.")
    return

def unpatch_model_cond_func(org_fn, self, *args, **kwargs):
    if hasattr(self.model, "diffusion_model") and hasattr(self.model.diffusion_model, "_deployable_module_quantized"):
...
model_patch_hijacker.register(ModelPatcher.unpatch_model, unpatch_model_oneflow, unpatch_model_cond_func)
