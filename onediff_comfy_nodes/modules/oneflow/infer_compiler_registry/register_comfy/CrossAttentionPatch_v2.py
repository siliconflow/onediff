    ...
    def __call__(self, q, k, v, extra_options):
        ...
        for i, callback in enumerate(self.callback):
            if sigma <= self.kwargs[i]["sigma_start"] and sigma >= self.kwargs[i]["sigma_end"]:
                out = out + callback(out, q, k, v, extra_options, **self.kwargs[i], **patch_kwargs[i])
    ...
    if unfold_batch:
        ...
        if ad_params is not None and ad_params["sub_idxs"] is not None:
            if isinstance(weight, torch.Tensor):
                ...
                # weight = weight.repeat_interleave(len(cond_or_uncond), dim=0) # repeat for cond and uncond
